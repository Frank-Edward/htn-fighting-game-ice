...
class HtnAI(object):
    ...
    def processing(self):
        ...
        if not self.isGameJustStarted:
            ...
            self.frameData = self.simulator.simulate(self.frameData, self.player, None, None, 17)#look ahead 15+2, other python ais like machete sample did this
        ...
        if not self.plan:  
            self.plan = plan(Act, self.Conditions)
            

        if self.cc.getSkillFlag(): #does not cover ariels, need something for inactionable
        ...
        self.inputKey.empty()
        self.cc.skillCancel()
        #print(self.action, "and cond : ", self.Conditions)
        self.cc.commandCall(self.plan)
